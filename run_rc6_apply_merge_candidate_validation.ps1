...
Write-Host "=== RC6 Apply Merge Candidate Validation ==="

Write-Host ""
Write-Host "=== Python tests ==="
python -m pytest tests/test_rc6_merge_candidate_apply_service.py tests/test_rc6_merge_candidate_apply_api_contract.py -q
python -m pytest -q
...
Write-Host ""
Write-Host "=== Inspect applied candidate ==="
docker compose -f docker-compose.prod.yml exec aici-db psql -U postgres -d aici -c "select id, signature, decision, status, reviewed_at, applied_at from merge_candidates where id = '$candidateId';"

Write-Host ""
Write-Host "=== RC6 Apply Merge Candidate Validation Passed ==="
