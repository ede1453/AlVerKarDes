...
Write-Host "=== RC69 Notification Outbox Retry Scheduler Validation ==="

Write-Host ""
Write-Host "=== Targeted tests ==="
python -m pytest tests/test_rc69_notification_outbox_retry_scheduler_service.py tests/test_rc69_notification_outbox_retry_scheduler_api.py tests/test_rc69_notification_outbox_retry_scheduler_openapi.py tests/test_rc69_notification_outbox_retry_scheduler_vertical_slice.py -q

Write-Host ""
Write-Host "=== Related outbox tests ==="
python -m pytest tests/test_rc68_notification_outbox_worker_service.py tests/test_rc68_notification_outbox_api_contract.py tests/test_rc67_notification_outbox_service.py tests/test_rc66_notification_idempotency_contract.py -q
...
Write-Host ""
Write-Host "=== Retry scheduler smoke ==="
Invoke-RestMethod -Method Post -Uri "http://localhost:8000/api/v1/notification-outbox/clear" | ConvertTo-Json -Depth 12

$payload = @{
    user_id = "rc69-docker-user"
    title = "RC69 docker outbox"
    message = "Retry scheduler smoke."
    payload = @{ source = "rc69-docker" }
...
Write-Host ""
Write-Host "=== RC69 Notification Outbox Retry Scheduler Validation Passed ==="
